...
v1_blueprint = Blueprint('v1', __name__, template_folder='templates')
...
def do_login():
    ...
    if username == "admin" and password == "secure123":
        ...
        return redirect(url_for('v1.intermediate'))
    return "Invalid credentials", 401

@v1_blueprint.route('/intermediate', methods=['GET'])
def intermediate():
    if not session.get("logged_in"):
        return redirect(url_for('v1.login'))
    return render_template('intermediate.html')

@v1_blueprint.route('/submit_intermediate', methods=['POST'])
...
@v1_blueprint.route('/download_doc', methods=['POST'])
def download_doc():
    data = request.json
    doc = Document()
