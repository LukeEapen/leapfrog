...
v2_blueprint = Blueprint('v2', __name__, template_folder='templates')
...
@v2_blueprint.route('/', methods=['GET', 'POST'])
def login():
...
def page1():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('v2.page2'))

    return render_template('page1_input.html')


@v2_blueprint.route('/page2', methods=['GET', 'POST'])
def page2():
    if not session.get('logged_in'):
        return redirect(url_for('v2.login'))
    ...
    if request.method == 'POST':
        ...
        if USING_REDIS:
            redis_client.setex(session_id, 3600, json.dumps(data))
        return redirect(url_for('v2.page3'))
...
@v2_blueprint.route('/page3', methods=['GET', 'POST'])
def page3():
    if not session.get('logged_in'): return redirect(url_for('v2.login'))
...
@v2_blueprint.route('/page4', methods=['GET', 'POST'])
def page4():
    if not session.get('logged_in'):
        return redirect(url_for('v2.login'))
...
@v2_blueprint.route("/download_doc", methods=["POST"])
def download_doc():
    doc = Document()
